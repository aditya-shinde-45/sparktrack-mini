...
const Header = ({ name, id }) => {
  ...
  const handleResetPassword = () => {
    // TODO: Call API here
    console.log("Previous:", prevPassword, "New:", newPassword);
    setShowModal(false);
    setPrevPassword("");
    setNewPassword("");
  };
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-gradient-to-b from-[#7B74EF] to-[#5D3FD3] p-3 sm:p-4 rounded-b-lg text-white flex justify-between items-center shadow-lg">
      ...
      <div className="flex items-center relative" ref={menuRef}>
        ...
        {showMenu && (
          <div
            className="absolute right-0 top-full mt-2 w-48 bg-white text-gray-800 rounded-xl shadow-xl ring-1 ring-black/5 animate-fadeIn"
          >
            <button
              onClick={() => {
                setShowModal(true);
  ...
  <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-80 animate-fadeIn">
      ...
      {/* Previous Password */}
      <div className="mb-3">
        <label className="block text-sm mb-1 text-gray-800">Previous Password</label>
        <input
          type="password"
          value={prevPassword}
      ...
      {/* New Password */}
      <div className="mb-4">
        <label className="block text-sm mb-1 text-gray-800">New Password</label>
        <input
          type="password"
          value={newPassword}
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          ...
          className="px-4 py-2 text-sm rounded-md loginbutton"
        >
          Update
        </button>
      </div>
